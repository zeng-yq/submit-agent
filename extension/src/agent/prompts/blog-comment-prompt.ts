...
export function buildBlogCommentPrompt(input: BlogCommentPromptInput): string {
  ...
  return [
    ...
    '10. Comment quality requirements:',
    '    - Must reference a specific concept, argument, or example from the page content',
    '    - Do NOT start with: "Great post", "Nice article", "Wonderful content", "Amazing", "Excellent", or similar generic openers',
    '    - Do NOT mention the product name more than once in the comment body',
    '    - Link anchor text (when using fallback HTML link) must be a natural phrase, NOT the exact product name or URL',
    '    - Length: minimum 80 characters, maximum 300 characters (excluding HTML tags)',
    '    - Must read as written by a genuine reader, not a marketer or AI',
    '',
    '## Output Format',
    '',
    'Return ONLY a JSON object. Keys MUST be the exact canonical_id values listed above.',
    'Do NOT use field labels, names, or any other identifiers as keys. Do NOT add extra fields.',
    '',
    'Required:',
    '- Every field listed above MUST have a corresponding key, even if the value is empty string ""',
    '- For fields you should skip, use empty string "" — never omit the key',
    '- For maxlength fields: count characters, truncate at the limit',
    '- Never use null or undefined as values',
    '',
    'Schema: { "<canonical_id>": "<value>", ... }',
    '',
